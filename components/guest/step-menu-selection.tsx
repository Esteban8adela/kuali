...
export function StepMenuSelection({ tripId, menus, locale }: StepMenuSelectionProps) {
  ...
  const canContinue =
    mode === "surprise" || mode === "custom" || (mode === "predefined" && !!selectedId);
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      <Card className="glass-card border-0">
        ...
        <CardContent className="space-y-6">
          <div className="rounded-lg border border-[#C4A052]/20 px-4 py-3">
            <div className="flex items-center justify-between">
              <Label htmlFor="season">{t("filterSeason")}</Label>
              <Switch id="season" checked={seasonOnly} onCheckedChange={setSeasonOnly} />
            </div>
            <p className="mt-3 text-sm leading-relaxed text-neutral-600">{t("offSeasonNote")}</p>
          </div>

          <div className="flex flex-wrap gap-2">
            <Button
              type="button"
              variant={mode === "predefined" ? "gold" : "outline"}
              size="sm"
          ...
          {(mode === "surprise" || mode === "custom") && (
            <p className="rounded-lg border border-[#C4A052]/30 bg-[#C4A052]/10 px-4 py-3 text-sm text-[#1B3A4B]">
              {mode === "surprise" ? t("surpriseNote") : t("customNote")}
            </p>
          )}
          ...
          <div
            className={cn(
              ...
              catalogDimmed && "pointer-events-none opacity-40"
            )}
            aria-hidden={catalogDimmed}
          >
                ...
                <div key={tier} className="space-y-3">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {tierMenus.map((menu) => {
                      ...
                      const selected = mode === "predefined" && selectedId === menu.id;
                      return (
                        <button
                          key={menu.id}
                          type="button"
                          onClick={() => selectPredefined(menu.id)}
                          className={cn(
                        ...
                        >
                          ...
                          <p className="mt-3 text-sm font-medium text-[#1B3A4B]">
                            {formatPrice(menu.price_adult_cents)} · {formatPrice(menu.price_child_cents)}{" "}
                            {t("childPrice")}
                          </p>
                        </button>
                      );
